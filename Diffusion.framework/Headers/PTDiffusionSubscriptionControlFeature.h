...
typedef NS_ENUM(NSInteger, PTDiffusionSubscriptionControlFeatureErrorCode) {
    /**
     ...
     @since 6.1
     */
    PTDiffusionSubscriptionControlFeatureErrorCode_SendToFilterRejected = 1,
};
...
/**
 ...
 @since 6.1
 */
extern NSString *const PTDiffusionSubscriptionControlFeatureErrorDomain;
...
/**
 ...
 for handling requests to subscribe to routing topics.

 The Subscription Control feature for a session can be obtained from the
 session's `subsciptionControl` property.
 ...
 permission, and `SELECT_TOPIC` permission for the path prefix of the topic
 selector used for subscription. The subscribed sessions will only be subscribed
 to matching topics for which they have `READ_TOPIC` permission.

 To unsubscribe other sessions, a session must have `MODIFY_SESSION` permission.

 To register a routing subscription handler the session needs `VIEW_SESSION`,
 ...
 the target session needs `READ_TOPIC` permission to both the routing topic
...
/**
 ...
 provided topic selector and for which the subscribed session has `READ_TOPIC`
 ...
 A session that does not have `SELECT_TOPIC` permission for a topic cannot
...
/**
 ...
 sessions has `READ_TOPIC` permission. The topic selector will be added to the
...
-(PTDiffusionStream *)addRoutingRequestHandler:(id<PTDiffusionSubscriptionRoutingRequestHandler>)handler
                             ...
                             completionHandler:(void (^)(PTDiffusionRegistration * _Nullable registration, NSError * _Nullable error))completionHandler;
